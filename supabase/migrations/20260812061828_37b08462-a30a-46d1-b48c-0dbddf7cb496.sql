-- 1. IDENTIFICAR E LIMPAR DUPLICATAS DO MÓDULO 1
-- Vamos manter o influenciador-ia-m1 e remover os outros que representam o Módulo 1
...
WHERE module_id IN (
    ...
    WHERE course_id IN (SELECT id FROM public.courses WHERE slug = 'influenciador-ia-tiktok-shop')
      AND slug IN ('modulo-1-mentalidade-e-nichos', 'mentalidade-e-nichos-lucrativos')
);
...
-- 2. REVISÃO DO MÓDULO 1 OFICIAL (influenciador-ia-m1)
UPDATE public.modules
SET 
    title = 'MÓDULO 1 — TikTok Shop: Mentalidade e Nichos Lucrativos',
...
Ter a fundação estratégica pronta, eliminando a dúvida sobre "o que vender" e focando 100% na execução técnica nos próximos módulos.',
    ...
    sort_order = 1
WHERE course_id IN (SELECT id FROM public.courses WHERE slug = 'influenciador-ia-tiktok-shop')
  AND slug = 'influenciador-ia-m1';

-- 3. INSERIR QUESTÕES PARA O MÓDULO OFICIAL
DELETE FROM public.questions WHERE module_id = (SELECT id FROM public.modules WHERE course_id IN (SELECT id FROM public.courses WHERE slug = 'influenciador-ia-tiktok-shop') AND slug = 'influenciador-ia-m1');

INSERT INTO public.questions (module_id, question, options, correct_answer, explanation)
VALUES 
(
    (SELECT id FROM public.modules WHERE course_id IN (SELECT id FROM public.courses WHERE slug = 'influenciador-ia-tiktok-shop') AND slug = 'influenciador-ia-m1'),
...
(
    (SELECT id FROM public.modules WHERE course_id IN (SELECT id FROM public.courses WHERE slug = 'influenciador-ia-tiktok-shop') AND slug = 'influenciador-ia-m1'),
    'O que caracteriza um conteúdo "nativo" no TikTok Shop?',
    '["Vídeos com logo da empresa em 100% do tempo", "Conteúdo que não parece anúncio e gera entretenimento", "Vídeos de 30 minutos explicando o produto", "Links de sites externos no meio do vídeo"]',
    'Conteúdo que não parece anúncio e gera entretenimento',
    'Se parecer um anúncio tradicional, o usuário pula. O conteúdo deve se misturar organicamente ao feed do usuário.'
),
(
    (SELECT id FROM public.modules WHERE course_id IN (SELECT id FROM public.courses WHERE slug = 'influenciador-ia-tiktok-shop') AND slug = 'influenciador-ia-m1'),
    'Qual destes nichos é considerado um "Oceano Azul" para IAs no TikTok Shop?',
    '["Venda de terrenos virtuais", "Gadgets de Tech e Home Setup", "Leilão de artes antigas", "Suporte técnico de software"]',
    'Gadgets de Tech e Home Setup',
    'A estética futurista da IA combina perfeitamente com a inovação desses produtos, gerando alta conversão.'
),
(
    (SELECT id FROM public.modules WHERE course_id IN (SELECT id FROM public.courses WHERE slug = 'influenciador-ia-tiktok-shop') AND slug = 'influenciador-ia-m1'),
    'O que deve ser feito antes de avançar para a criação técnica do influenciador?',
    '["Comprar uma câmera profissional", "Definir o nicho e o Plano de Voo", "Contratar uma agência de modelos", "Registrar uma patente internacional"]',
    'Definir o nicho e o Plano de Voo',
    'A estratégia deve preceder a execução técnica para garantir que o influenciador tenha um propósito comercial claro.'
);